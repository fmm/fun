...
bool valid(vector<Int>& a) {
	...
	if(a.back()>1e9) return false;
	rep(i,a.size()-1) if(a[i]==a[i+1]) return false;
	return true;
}

bool pop(map<Int,int> &M, Int x) {
	if(M.count(x) == 0) return false; else M[x]--;
	if(M[x] <= 0) M.erase(x);
	return true;
}
		...
		if(n==3) {
			...
			for(int candidate=2;candidate<lim;candidate++) {
				...
				for(int p=3;p<n;p++) {
					...
					aa[p]=X/A;
					for(int i=0;i<p;i++) if(!pop(M,aa[i]*aa[p])) {
						ok=false;
						goto fail;
					}
				}
fail:
				if(ok && valid(aa) && aa<a) a=aa;
			}
		}
		printf("Case %d:",testcase);
		rep(i,n) printf(" %d",int(a[i])); puts("");
	}
	return 0;
}
